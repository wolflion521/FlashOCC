...
class BEVDet(CenterPoint):
    ...
    def prepare_inputs(self, inputs):
        ...
        # 使用 CPU 求逆避免 cuSOLVER 错误
        try:
            # 尝试在 GPU 上求逆
            if hasattr(torch.linalg, 'inv'):
                global2keyego = torch.linalg.inv(keyego2global.float())   # (B, 1, 4, 4)
            else:
                global2keyego = torch.inverse(keyego2global.float())   # fallback
        except RuntimeError as e:
            # GPU cuSOLVER 失败，使用 CPU 求逆
            print(f'Warning: GPU matrix inversion failed, using CPU fallback')
            keyego2global_cpu = keyego2global.float().cpu()
            global2keyego = torch.inverse(keyego2global_cpu).cuda()   # (B, 1, 4, 4)
